...
impl Kind {
    ...
    pub fn detect(body: &[u8]) -> Option<Kind> {
        if Self::contains(body, b"pid")
            || Self::contains(body, b"Query Time (s)")
            || Self::contains(body, b"PostgreSQL JDBC Driver")
        {
            Some(Kind::PGSQL)
        } else if Self::contains(body, b"Microsoft JDBC Driver for SQL Server")
            || Self::contains(body, b"Command")
            || Self::contains(body, b"Logical Reads")
            || Self::contains(body, b"Wait Type")
            || Self::contains(body, b"Wait Resource")
            || Self::contains(body, b"Blocked by")
        {
            Some(Kind::MSSQL)
        } else {
            None
...
mod test {
    use crate::{
        ...
        parser::stuckquery::{Kind, MSSQLTable, PGSQLQuery, PGSQLTable},
        util,
    };
    ...
    fn stuckquery_pgsql_parse_table() {
        ...
        assert_eq!(kind, Some(Kind::PGSQL));
        let table = StuckQueryIterator::new(kind.unwrap(), &map).next();
    ...
    fn stuckquery_pgsql_parse_full_file() {
        ...
        let kind = Kind::detect(&map);
        assert_eq!(kind, Some(Kind::PGSQL));
        for table in StuckQueryIterator::new(kind.unwrap(), &map) {
            let table = PGSQLTable::try_from(table);
            assert!(table.is_ok(), "Error: {:?}", table.unwrap_err());
            let table = table.unwrap();
