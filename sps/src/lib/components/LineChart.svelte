...
<script lang="ts">
	...
	const visibleSeries = $derived.by(() => {
		...
		return series.map((s) => ({
			...
			points: s.points.filter(
				(p) => p.timestamp >= z0 && p.timestamp <= z1,
			),
		}));
	});
	...
	const x = $derived(
		scaleUtc()
			.domain(
				zoom ??
					(allPoints.length > 0
						? (extent(allPoints, (p) => p.timestamp) as [
								number,
								number,
							])
						: [Date.now() - 1, Date.now()]),
			...
			.range([
				marginLeft,
				Math.max(marginLeft + 1, width - margin.right),
			]),
	);
	...
	const y = $derived(
		scaleLinear()
			...
			.nice()
			.range([
				Math.max(margin.top + 1, height - margin.bottom),
				margin.top,
			]),
	);
	...
	const rendered = $derived(
		visibleSeries.map((s) => ({
			series: s,
			bins:
				s.points.length > plotWidth * 2 ? binPerPixel(s.points) : null,
		})),
	);
	...
	function onpointerup(event: PointerEvent) {
		if (dragStart !== null && dragCurrent !== null && dragging) {
			...
			if (target)
				onpointselect?.(target.series.id, target.point.timestamp);
		}
		dragStart = null;
		dragCurrent = null;
	}
	...
	{:else if width > 0 && height > 0}
		...
		{#if zoom}
			<button
				...
				onclick={() => (zoom = null)}
				title="Reset zoom (or double-click the chart)">reset</button
			>
		{/if}
