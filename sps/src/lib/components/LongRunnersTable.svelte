...
<div class="table">
	...
	<div class="rows">
		{#each rows as row (row.key)}
			...
			>
				...
				<span class="col-num mono"
					>{row.maxMs === null
						? "—"
						: formatDuration(row.maxMs)}</span
				>
				<span class="col-num mono" class:bad={row.flagCount > 0}>
					{row.flagCount > 0
						? `${row.flagCount} ${row.flagLabel}`
						: "—"}
				</span>
			...
			{#if expanded === row.key}
				<div class="expand">
					<dl>
						<dt>First seen</dt>
						<dd class="mono">
							{formatTimestamp(timeFormat, row.firstSeen)}
						</dd>
						<dt>Last seen</dt>
						<dd class="mono">
		...
		{:else}
			<p class="empty">
				No execution appears in more than one snapshot — nothing was
				stuck across dumps.
			</p>
		{/each}
	</div>
