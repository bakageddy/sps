<script lang="ts">
	...
	function label(t: StuckThread): string {
		if (t.request === null)
			return t.name || "(warning lost — unknown request)";
		try {
		...
		{#each visible as thread}
			...
			>
				...
				<span class="col-status">
					<span
						class="badge"
						class:done={thread.end !== null}
						class:open={thread.end === null}
					>
						{thread.end !== null ? "done" : "stuck"}
					</span>
				</span>
				<span class="col-tid mono">{thread.tid}</span>
				<span class="col-start mono"
					>{formatTimestamp(timeFormat, bounds(thread)[0])}</span
				>
				<span class="col-duration mono"
					>{formatDuration(thread.duration)}</span
				>
			</button>
		{:else}
			<p class="empty">
