	...
	{#each chains as [head, victims]}
		<div class="chain">
			<div class="head-row">
				...
				<span class="count"
					>blocking {victims.length} session{victims.length === 1
						? ""
						: "s"}</span
				>
				<button
					...
					><Icon
						name={copiedChain === head ? "check" : "copy"}
						size={12}
					/></button
				>
			</div>
			{#each victims as v}
				<button
					...
					onclick={() =>
						(expanded = expanded === rowKey(v) ? null : rowKey(v))}
				>
					<span class="elbow">└─</span>
					<span class="mono session">{v.sessionId}</span>
					<span class="mono wait" title={v.waitResource ?? undefined}
						>{v.waitType ?? "—"}</span
					>
					<span class="mono dur"
						>{formatDuration(v.waitDuration)}</span
					>
					<span class="mono query"
						>{v.blockerQueryOrMostRecentQuery}</span
					>
				</button>
				{#if expanded === rowKey(v)}
					<div
						class="expand"
						style:margin-left="{12 + v.level * 22}px"
					>
						<dl>
							<dt>Blocked by</dt>
							<dd class="mono">session {v.blockingSessionId}</dd>
							<dt>Txn</dt>
							<dd class="mono">{v.txnId}</dd>
							<dt>Wait resource</dt>
							<dd class="mono">{v.waitResource ?? "—"}</dd>
							<dt>Offsets</dt>
							<dd class="mono">
								{v.statementStartOffset} → {v.statementEndOffset}
							</dd>
						</dl>
