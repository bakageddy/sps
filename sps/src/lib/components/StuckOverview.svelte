<script lang="ts">
	...
	const pct = (t: number) =>
		((t - domain[0]) / (domain[1] - domain[0])) * 100;
	...
	const visibleBars = $derived(
		lanes.placed.filter(
			({ bar }) =>
				bar.timestamp <= domain[1] &&
				bar.timestamp + bar.durationMs >= domain[0],
		),
	);
	...
	const laneCapacity = $derived(
		Math.max(1, Math.floor(stripHeight / LANE_PITCH)),
	);
	const rendered = $derived(
		visibleBars.filter(({ lane }) => lane < laneCapacity),
	);
	...
	const isSelected = (bar: StuckBar) =>
		selected !== null &&
		selected.tid === bar.tid &&
		selected.timestamp === bar.timestamp;
	...
	const dragging = $derived.by(() => {
		if (dragFracStart === null || dragFracCurrent === null || !strip)
			return false;
		return (
			Math.abs(dragFracCurrent - dragFracStart) * strip.clientWidth >
			DRAG_THRESHOLD
		);
	});
	...
	<div class="axis">
		{#each ticks as tick, i (i)}
			<span class="tick" style:left="{tick.pct}%"
				>{formatTimestamp(timeFormat, tick.t)}</span
			>
		{/each}
	</div>
		...
		{#each rendered as { bar, lane }}
			...
			<button
				...
				style:width="{Math.max(
					0.3,
					pct(bar.timestamp + bar.durationMs) - pct(bar.timestamp),
				)}%"
				style:top="{lane * LANE_PITCH +
					(LANE_PITCH - LINE_HEIGHT) / 2}px"
