<script lang="ts">
	...
	function applySpan(ms: number) {
		...
		const center =
			view === null ? domain[1] - span / 2 : (view[0] + view[1]) / 2;
	...
	const presetActive = (ms: number) =>
		view !== null &&
		Math.abs(windowSpan - ms) < 500 &&
		Math.abs(view[1] - domain[1]) < 500;
	...
	const overviewSelected = $derived(
		selected === null
			? null
			: { tid: selected.tid, timestamp: bounds(selected)[0] },
	);
	...
	async function refresh() {
		try {
			...
			threads = await cached("stuckthread_listview", () =>
				stuckthreadListview(),
			);
		} catch (e) {
	...
	async function onselect(thread: StuckThread) {
		...
		try {
			const frames = await cached(
				`stuckthread_trace:${tid}:${timestamp}`,
			...
			trace = {
				...
				tid,
				timestamp,
				frames: frames.length === 0 ? null : frames,
			};
		} catch (e) {
	...
	async function copyEpisode() {
		...
		const active = [selected.activeStart, selected.activeEnd].filter(
			(n) => n !== null,
		);
		const lines = [
		...
		if (trace.status === "ready" && trace.frames !== null) {
			...
			for (const frame of trace.frames)
				lines.push(`  at ${frame.method} (${frame.source})`);
		}
		try {
	...
	<div class="toolbar">
		...
		<span class="chips" role="group" aria-label="Status filter">
			<button
				...
				onclick={() => (statusFilter = Status.All)}>All</button
			>
			<button
				class:active={statusFilter === Status.Done}
				onclick={() => (statusFilter = Status.Done)}>Completed</button
			>
			<button
				class:active={statusFilter === Status.Stuck}
				onclick={() => (statusFilter = Status.Stuck)}>Stuck</button
			>
		</span>
		...
		<span class="chips" role="group" aria-label="Table mode">
			<button
				...
				onclick={() => (viewMode = ViewMode.Episodes)}>Episodes</button
			>
			<button
				class:active={viewMode === ViewMode.Paths}
				onclick={() => (viewMode = ViewMode.Paths)}>Paths</button
			>
		</span>
		...
		<span class="zoom" role="group" aria-label="Time window">
			...
			<button
				...
				disabled={view === null}
				title="Zoom out"
				aria-label="Zoom out"><Icon name="zoomOut" size={13} /></button
			>
			<button
				class="zbtn"
				onclick={() => applySpan(windowSpan / 2)}
				...
				title="Zoom in"
				aria-label="Zoom in"><Icon name="zoomIn" size={13} /></button
			>
			<button
				class="reset"
				onclick={() => (view = null)}
				disabled={view === null}
				title="Reset zoom to full range">reset</button
			>
		</span>
