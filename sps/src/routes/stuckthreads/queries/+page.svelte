<script lang="ts">
	...
	async function selectThread(thread: StuckThread | null) {
		...
		try {
			...
			if (
				selectedThread === null ||
				threadKey(selectedThread) !== threadKey(thread)
			)
				return;
	...
	function loadSnapshot(snap: SnapshotRow | null) {
		...
		const guard = () =>
			selectedSnap !== null && snapshotKey(selectedSnap) === key;
		if (snap.kind === "mssql") {
			Promise.all([
	...
	function onselectepisode(thread: StuckThread) {
		...
		selectedSnap =
			nearest !== null &&
			Math.abs(nearest.timestamp - targetOf(thread)) <=
	...
	function onselectsnapshot(snap: SnapshotRow) {
		...
		void selectThread(
			nearest !== null &&
				Math.abs(nearest.timestamp - snap.timestamp) <=
					threshold.value * 1000
				? nearest.t
				: null,
		);
