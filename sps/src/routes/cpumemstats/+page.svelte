<script lang="ts">
	...
	import PathTotalsTable, {
		type PathTotal,
	} from "$lib/components/PathTotalsTable.svelte";
	import LineChart, {
		type LineSeries,
			...
			: [
					{
						...
						label: selectedLabel,
						points:
							metric === Metric.Cpu ? series.cpu : series.memory,
					},
				],
	);
	...
	async function onselectdump(timestamp: number, preserveChart = false) {
		...
		const [cpu, memory] = await Promise.allSettled([
			cached(`cpumem_cpu_processes:${timestamp}`, () =>
				cpuMemCpuProcesses(timestamp),
			),
			cached(`cpumem_mem_processes:${timestamp}`, () =>
				cpuMemMemoryProcesses(timestamp),
			),
		]);
	...
	async function onselectprocess(pid: number) {
		...
		try {
			series = await cached(`cpumem_series:${pid}`, () =>
				cpuMemSeries(pid),
			);
		} catch (e) {
	...
	async function onselectpath(row: PathTotal) {
		...
		try {
			series = await cached(
				`cpumem_path_series:${row.byPath}:${row.label}`,
	...
	<div class="toolbar">
		<div class="toggle" role="group" aria-label="Metric">
			<button
				...
				onclick={() => (metric = Metric.Cpu)}>CPU</button
			>
			<button
				class:active={metric === Metric.Memory}
				onclick={() => (metric = Metric.Memory)}>Memory</button
			>
		</div>
