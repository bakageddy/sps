...
CREATE
OR REPLACE TYPE main.cpumonitoring_thread_state AS ENUM (
  'RUNNABLE',
  'NEW',
  'BLOCKED',
  'WAITING',
...
CREATE
OR REPLACE TYPE main.thread_state AS ENUM (
  'RUNNABLE',
  'NEW',
  'BLOCKED',
  'WAITING',
...
CREATE
OR REPLACE TYPE main.stuckquery_pgsql_state AS ENUM ('active', 'idle in transaction');

CREATE
OR REPLACE TYPE main.stuckquery_mssql_status AS ENUM (
  'runnable',
  'running',
  'rollback',
  'sleeping',
...
CREATE
OR REPLACE TYPE main.connectiondump_cause AS ENUM (
  'High CPU',
...
CREATE TABLE IF NOT EXISTS main.threaddump (timestamp UBIGINT NOT NULL,);
