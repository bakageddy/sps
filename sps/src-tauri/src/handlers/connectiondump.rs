use crate::{
    AppState, handlers::types::ConnectionDumpSnapshot, parser::connectiondump::{Signal, Stats}, store,
};
use std::sync::Mutex;
...
pub fn connectiondump_snapshots(
    ...
    let result = store::connectiondump::get_connectiondump_snapshots(&cnx)
        .map_err(|e| format!("Error during fetching connection dump snapshots"));
    result
}
