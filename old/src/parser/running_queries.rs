use std::{iter::Peekable, net::Ipv4Addr};
...
use crate::{
    ...
    ingest::{kind::DBKind, running_queries::Entry},
...
impl<'a> State<'a> {
    pub fn map_to_row(&self) -> &'static str {
        match self {
            Self::Active => "active",
            Self::Idle => "idle",
...
pub struct RunningQueryParser<'a, T>(pub Peekable<T>, pub DBKind)
where
...
where
    T: Iterator<Item = Entry<'a>>,
{
    pub fn new(kind: DBKind, iter: T) -> Self
    where
        T: Iterator<Item = Entry<'a>>,
    {
...
impl<'a> TryFrom<(DBKind, Entry<'a>, Entry<'a>)> for RunningQueryTable<'a> {
    type Error = Error;

    fn try_from(value: (DBKind, Entry<'a>, Entry<'a>)) -> Result<Self, Self::Error> {
        let value = match value.0 {
            DBKind::PGSQL => Self::PGSQL(PGSQLRunningQueryTable::try_from((value.1, value.2))?),
            DBKind::MSSQL => Self::MSSQL(MSSQLRunningQueryTable::try_from((value.1, value.2))?),
        };
        Ok(value)
    }
}

impl<'a> SchemaMapper for PGSQLQuery<'a> {
    // pid,
    // query_time_ms,
    // txn_time_ms,
    // last_state_change,
    ...
    fn map_to_row(&self) -> Self::Item {
        (
            ...
            self.client_port,
            self.state.map_to_row(),
            self.waiting,
            self.client_address.map(|addr| addr.to_bits()),
            self.db_name,
            self.query,
...
impl<'a> SchemaMapper for Status<'a> {
...
impl<'a> SchemaMapper for SPWho2Status<'a> {
    type Item = &'static str;
