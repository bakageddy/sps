...
use crate::parser::{
    ...
    running_queries::{MSSQLCurrentRunningQueryTable, PGSQLRunningQueryTable, SPWho2Table},
...
impl Store {
    ...
    pub fn insert_running_query_pgsql(
        appender: &mut Appender,
        tables: Vec<PGSQLRunningQueryTable>,
    ) -> Result<()> {
        for table in tables {
            appender.append_rows(table.queries.iter().map(|query| query.map_to_row()).map(
                |row| {
                    (
                        table.timestamp,
                        row.0,
    ...
    pub fn insert_running_query_mssql(
        appender: &mut Appender,
        tables: Vec<MSSQLCurrentRunningQueryTable>,
    ) -> Result<()> {
        for table in tables {
            for query in table.queries {
    ...
    pub fn insert_running_query_spwho2(
        appender: &mut Appender,
        tables: Vec<SPWho2Table>,
    ) -> Result<()> {
        for table in tables {
            appender.append_rows(table.entries.iter().map(|e| {
                (
                    table.timestamp,
                    e.spid,
                    e.status.map_to_row(),
